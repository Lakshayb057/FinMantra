...
export default function AdminDashboard({ navigateTo, theme, toggleTheme }) {
  ...
  const getOptimalSqlQuery = (bankName, category) => {
    ...
    if (cat.includes('FINAL') && (cat.includes('APPROVED') || cat.includes('CARD'))) {
      if (bank.includes('KIWI')) {
        catCond = `(LOWER(mis_status) IN ('card created', 'card_created', 'card generated') OR LOWER(COALESCE(mis_data->>'Card_Created','')) IN ('1','yes','true') OR LOWER(COALESCE(mis_data->>'card_created','')) IN ('1','yes','true') OR LOWER(COALESCE(mis_data->>'current_state','')) IN ('card created', 'card_created') OR LOWER(COALESCE(mis_data->>'card_activation_status','')) IN ('active','yes','1') OR UPPER(mis_data::text) LIKE '%"CARD_CREATED":"1"%' OR UPPER(mis_data::text) LIKE '%"CARD_CREATED": "1"%' OR UPPER(mis_data::text) LIKE '%"CARD_CREATED":"YES"%' OR UPPER(mis_data::text) LIKE '%"CARD_CREATED": "YES"%') AND NOT (UPPER(mis_status) LIKE '%REJECT%' OR UPPER(mis_status) LIKE '%DECLINE%' OR UPPER(mis_status) LIKE '%SOFT%' OR UPPER(mis_status) LIKE '%VKYC%' OR UPPER(mis_status) LIKE '%IPA%' OR UPPER(mis_status) LIKE '%PENDING%')`;
      } else {
