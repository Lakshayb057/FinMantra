...
export default function AdminDashboard({ navigateTo, theme, toggleTheme }) {
  ...
  const getOptimalSqlQuery = (bankName, category) => {
    ...
    if (cat.includes('FINAL') && (cat.includes('APPROVED') || cat.includes('CARD'))) {
      if (bank.includes('KIWI')) {
        catCond = `(UPPER(mis_data::text) LIKE '%CARD%CREATE%' OR UPPER(mis_data::text) LIKE '%CARD_CREATED%' OR UPPER(COALESCE(mis_status,'')) LIKE '%CARD%CREATE%' OR UPPER(COALESCE(mis_status,'')) LIKE '%CARD_CREATED%' OR LOWER(COALESCE(mis_data->>'card_activation_status','')) IN ('active','yes','1')) AND NOT (UPPER(COALESCE(mis_status,'')) LIKE '%REJECT%' OR UPPER(COALESCE(mis_status,'')) LIKE '%DECLINE%')`;
      } else {
