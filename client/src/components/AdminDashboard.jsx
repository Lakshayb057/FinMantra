                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
                        <button
                          ...
                          onClick={() => setAudienceFormData({
                            ...
                            bank_name: 'KIWI',
                            mis_category: 'FINAL APPROVED',
                            sql_filter: `SELECT id, urn, full_name, phone, email, card_bank, mis_status FROM leads WHERE mis_status IS NOT NULL AND (UPPER(COALESCE(card_bank,'')) LIKE '%KIWI%' OR UPPER(COALESCE(mis_data->>'mis_bank_name','')) LIKE '%KIWI%' OR UPPER(COALESCE(card_name,'')) LIKE '%KIWI%' OR LOWER(source) = 'kiwi' OR mis_data->>'kiwi_winning_bank' IS NOT NULL OR UPPER(mis_data::text) LIKE '%KIWI%') AND (UPPER(COALESCE(mis_status,'')) LIKE '%CARD%' OR UPPER(COALESCE(mis_status,'')) LIKE '%CREATE%' OR UPPER(COALESCE(mis_status,'')) LIKE '%APPROV%' OR UPPER(mis_data::text) LIKE '%CARD%CREATE%' OR UPPER(mis_data::text) LIKE '%CARD_CREATED%' OR UPPER(mis_data::text) LIKE '%"CARD_CREATED":"1"%' OR UPPER(mis_data::text) LIKE '%"CARD_CREATED":1%' OR UPPER(mis_data::text) LIKE '%"CARD_CREATED":"YES"%')`
                          })}
