...
app.post('/api/leads/upload-mis', authenticateToken, upload.single('file'), async (req, res) => {
  ...
  if (updates.length > 0) {
    ...
    // Trigger Meta CAPI Purchase Events & Custom Audience Pushes for Final Approved leads asynchronously
    if (updatedLeads && updatedLeads.length > 0) {
      setTimeout(async () => {
        ...
        await syncLeadsToMetaCustomAudiences(updatedLeads);
      }, 100);
    }
  }
...
// --- Meta Audiences & CAPI Routes ---

// Get all Meta Custom Audiences
app.get('/api/meta/audiences', authenticateToken, requireAdmin, async (req, res) => {
  try {
...
app.post('/api/meta/audiences', authenticateToken, requireAdmin, async (req, res) => {
  try {
      ...
      try {
        ...
        const metaRes = await fetch(metaUrl, {
          ...
          body: JSON.stringify({
            name: name.trim(),
            subtype: 'CUSTOM',
            description: description || 'FinMantra Final Approved Leads Audience',
    ...
    const newAudience = await db.createMetaAudience({
      name: name.trim(),
      description: description || '',
      meta_audience_id: metaAudienceId,
      bank_name: bank_name || 'ALL',
      mis_category: mis_category || 'FINAL APPROVED',
      sql_filter: sql_filter || '',
      auto_push: auto_push !== undefined ? auto_push : true
    });

    res.json({ success: true, audience: newAudience });
  } catch (err) {
...
app.put('/api/meta/audiences/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const updated = await db.updateMetaAudience(req.params.id, req.body);
    if (!updated) {
      return res.status(404).json({ error: 'Audience not found' });
    }
    res.json({ success: true, audience: updated });
  } catch (err) {
    console.error('[API Error] PUT /api/meta/audiences/:id:', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.delete('/api/meta/audiences/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (audience && audience.meta_audience_id) {
      const settings = await db.getSettings();
      const accessToken = getSettingVal(settings, 'meta_access_token', 'META_ACCESS_TOKEN');
      if (accessToken) {
        try {
          await fetch(`https://graph.facebook.com/v20.0/${audience.meta_audience_id}?access_token=${accessToken}`, {
            method: 'DELETE'
          });
        } catch (e) {}
      }
    }
    await db.deleteMetaAudience(req.params.id);
    res.json({ success: true, message: 'Audience deleted successfully' });
  } catch (err) {
    console.error('[API Error] DELETE /api/meta/audiences/:id:', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/meta/audiences/:id/sync', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Sync process completed. Dispatched ${leads.length} lead record(s) to Meta Custom Audience '${audience.name}'.`,
      syncedCount: leads.length
    });
  } catch (err) {
    console.error('[API Error] POST /api/meta/audiences/:id/sync:', err.message);
    res.status(500).json({ error: err.message });
  }
});

// Get Audience History Logs
app.get('/api/meta/audiences/logs', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const logs = await db.getAudienceHistoryLogs();
    res.json({ success: true, logs });
  } catch (err) {
    console.error('[API Error] GET /api/meta/audiences/logs:', err.message);
    res.status(500).json({ error: err.message });
  }
});
