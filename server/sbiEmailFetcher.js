  ...
  try {
    ...
    try {
      ...
      for await (const message of messages) {
        ...
        candidates.push({ uidStr, sbiUidKey, subject, fromAddr });
      }

      // Phase 2: Fetch full message source specifically for each collected candidate UID
      for (const cand of candidates) {
        const { uidStr, sbiUidKey, subject, fromAddr } = cand;
        ...
        try {
          ...
          console.error(`[SBI Email Fetcher] Error fetching message UID ${uidStr}:`, fetchErr.message);
          continue;
        }
